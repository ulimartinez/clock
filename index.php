	...
	<body>
		...
		if ($('#digit-four').val()) {
			...
			'clock' : "true"
				...
				$('#myModalLabel').html(welcome);

				$('.modal-body').html(status + " " + Date());
				if ($(status).text() === 'OUT') {
					var total = parseInt(data.today) + data.time;
					console.log(total);
					$('.modal-body').append('<p>' + timeIn(data.time) + '</p>');
					$('.modal-body').append('<p>Total today: ' + timeIn((total)) + '</p>');
				}
				timeoutID = window.setTimeout(function() {
				$('#myModal').modal('hide');
				}, 6000);
